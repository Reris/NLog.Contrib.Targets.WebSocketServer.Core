...
namespace Owin.WebSocket
{
    public abstract class WebSocketConnection
    {
        ...
        protected IWebSocket mWebSocket { get; private set; }
        ...
        public WebSocketConnection(IHttpContextAccessor httpContextAccessor, int maxMessageSize = 1024 * 64)
            : this(maxMessageSize)
        {
        ...
        public virtual bool AuthenticateRequest(HttpRequest request)
        {
            return true;
        }
        ...
        /// <param name="request">Websocket request</param>
        /// <returns>True if the request is authenticated else false to throw unauthenticated and deny the connection</returns>
        public virtual Task<bool> AuthenticateRequestAsync(HttpRequest request)
        {
        ...
        internal async Task AcceptSocketAsync(IDictionary<string, object> owinContext, IDictionary<string, string> argumentMatches)
        {
            var httpContext = _httpContextAccessor.HttpContext;
            var environment = new OwinEnvironment(httpContext);
            var accept = environment.Get<Action<IDictionary<string, object>, Func<IDictionary<string, object>, Task>>>("websocket.Accept");
            if (accept == null)
            {
                // Bad Request
                httpContext.Response.StatusCode = 400;
                await httpContext.Response.WriteAsync("Not a valid websocket request");
                return;
            }

            Arguments = new Dictionary<string, string>(argumentMatches);

            var responseBuffering = environment.Get<Action>("server.DisableResponseBuffering");
            if (responseBuffering != null)
                responseBuffering();

            var responseCompression = environment.Get<Action>("systemweb.DisableResponseCompression");
            ...
            if (AuthenticateRequest(httpContext.Request))
            {
                var authorized = await AuthenticateRequestAsync(httpContext.Request);
                if (authorized)
                {
